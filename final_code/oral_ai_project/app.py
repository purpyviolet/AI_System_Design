...
# 直接使用绝对路径确保文件位置的准确性
UPLOAD_FOLDER = r'D:\华南理工\工导2.0\oral_ai_project\oral_ai_project\static\uploads'
if not os.path.exists(UPLOAD_FOLDER):
    os.makedirs(UPLOAD_FOLDER)

# 历史记录文件路径
HISTORY_FILE = 'D:\\华南理工\\工导2.0\\oral_ai_project\\oral_ai_project\\database\\history.json'

# 标签保存目录
LABELS_DIR = {
    'tongue': 'D:\\华南理工\\工导2.0\\oral_ai_project\\oral_ai_project\\database\\labels\\tongue',
    'tooth': 'D:\\华南理工\\工导2.0\\oral_ai_project\\oral_ai_project\\database\\labels\\tooth'
}
...
# 确保所有必要的目录存在
for dir_path in [UPLOAD_FOLDER, 'D:\\华南理工\\工导2.0\\oral_ai_project\\oral_ai_project\\database', HISTORY_FILE.rsplit('/', 1)[0]] + list(LABELS_DIR.values()):
...
if __name__ == '__main__':
    app.run(host='192.168.193.30', port=8080, debug=True)
